    ...
    for i in reuslt:
        if i[11] ==0:
            ...
            if date_difference == 90:
                message ="你的客户:\n"+"{:<20}{:<20}{:<20}\n".format( '合同编号', '客户名称','到期日') + ('-' * 100) + "\n"+"{:<20}{:<20}{:<20}\n".format(i[0], i[1],i[10].strftime('%Y-%m-%d'))+ "维保还有90天到期"
                receivers =list(test_user_manager().get_email_username(i[3])[0])
                ...
                message = "你的客户:\n" + "{:<20}{:<20}{:<20}\n".format('合同编号', '客户名称', '到期日') + (
                            '-' * 100) + "\n" + "{:<20}{:<20}{:<20}\n".format(i[0], i[1], i[10].strftime('%Y-%m-%d')) + "维保还有60天到期"
                receivers = list(test_user_manager().get_email_username(i[3])[0])
                Email().send_email(receivers, message, "维保到期提醒")
                tips = Tipsdb().tips_contract_id_get(i[0])
                if len(tips) == 0:
                ...
                message = "你的客户:\n" + "{:<20}{:<20}{:<20}\n".format('合同编号', '客户名称', '到期日') + (
                        '-' * 100) + "\n" + "{:<20}{:<20}{:<20}\n".format(i[0], i[1], i[10].strftime('%Y-%m-%d')) + "维保还有30天到期"
                receivers = list(test_user_manager().get_email_username(i[3])[0])
                Email().send_email(receivers, message, "维保到期提醒")
                tips = Tipsdb().tips_contract_id_get(i[0])
                if len(tips) == 0:
...
if __name__ == '__main__':
    ...
    model_data = ReadHandle(f'{file}/system/configure/saleman_mail_configure.yaml').yaml_files_read()
    tips_hours = model_data["tips_email"]["hour"]
    tips_min =model_data["tips_email"]["minute"]
    scheduler.add_job(func=tips_email, id='1', trigger='cron',hour=tips_hours,minute=tips_min)
    if model_data["month_report"]["open"] == 0:
